...
export default function ManageProducts() {
  ...
  const [activeTab, setActiveTab] = useState('catalog'); // 'catalog' | 'activate' | 'emergency'

  // Activate form state
  const [actForm, setActForm] = useState({
    memberId: '', serviceId: '', txRef: '', treeQty: 1
  });
  ...
  const [svcForm, setSvcForm] = useState({
    ...
    baseFee: '', subsequentFee: 0, totalMonths: 12, initialBonusUnits: 0
  });
  ...
  // Monthly cycle scheduler
  const [cycleLoading, setCycleLoading] = useState(false);
  const [cycleResult,  setCycleResult]  = useState(null); // { success, summary, error }
  ...
  const handleEmergency = async (e) => {
    e.preventDefault();
    setEmgLoading(true); setEmgMsg(null);
    try {
  ...
  const handleCreateService = async (e) => {
    ...
    try {
      ...
      setSvcForm({ name: '', description: '', type: 'SUBSCRIPTION', baseFee: '', subsequentFee: 0, totalMonths: 12, initialBonusUnits: 0 });
      load();
    } catch { /* silent */ }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-white rounded-xl border border-slate-200 p-4 gap-3">
        <div>
          <h2 className="text-sm font-semibold text-slate-800">Product Management</h2>
          <p className="text-xs text-slate-400 mt-0.5">{services.length} services | {members.filter(m => m.status === 'active').length} active members</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* New Service */}
          <button onClick={() => setShowCreateSvc(true)} className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">
        ...
        <div className={`rounded-xl border p-4 ${cycleResult.success ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
          {cycleResult.success ? (
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-4 mt-2 text-xs text-emerald-700">
                  ...
                  <span>🆕 Tickets Created: <strong>{cycleResult.summary?.ticketsGenerated ?? '—'}</strong></span>
                  <span>⏭ Skipped: <strong>{cycleResult.summary?.accountsSkippedOrAlreadyProcessed ?? '—'}</strong></span>
                </div>
              </div>
              <button onClick={() => setCycleResult(null)} className="ml-auto shrink-0 text-emerald-500 hover:text-emerald-700 transition">
            ...
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setCycleResult(null)} className="ml-auto text-red-400 hover:text-red-600 transition">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
          )}
      ...
      <div className="flex border-b border-slate-200 bg-white rounded-t-xl">
        {[
          ...
          { id: 'activate', label: 'Activate Member', icon: '✅' },
          { id: 'emergency', label: 'Emergency Request', icon: '🚨' },
        ].map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium transition ${activeTab === t.id ? 'text-emerald-700 border-b-2 border-emerald-500 bg-emerald-50/50' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'}`}>
          ...
          {services.length > 0 ? services.map(s => (
            <div key={s._id} className="p-5 hover:bg-slate-50/60 transition">
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full border shrink-0 ${s.type === 'SUBSCRIPTION' ? 'bg-violet-50 text-violet-700 border-violet-100' : 'bg-green-50 text-green-700 border-green-100'}`}>
                  {s.type}
                </span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                <div className="bg-slate-50 px-3 py-2 rounded border border-slate-100">
                  <p className="text-slate-400 font-medium">Base Fee</p>
                  <p className="text-slate-800 font-semibold">₹{s.baseFee?.toLocaleString('en-IN')}</p>
                </div>
                {s.type === 'ON_DEMAND' && s.subsequentFee > 0 && (
                  <div className="bg-slate-50 px-3 py-2 rounded border border-slate-100">
                    <p className="text-slate-400 font-medium">Extra Unit</p>
                    <p className="text-slate-800 font-semibold">₹{s.subsequentFee?.toLocaleString('en-IN')}</p>
                  </div>
                )}
                {s.type === 'SUBSCRIPTION' && (
                  <>
                    <div className="bg-slate-50 px-3 py-2 rounded border border-slate-100">
                      ...
                      <p className="text-slate-800 font-semibold">{s.totalMonths} months</p>
                    </div>
                    <div className="bg-slate-50 px-3 py-2 rounded border border-slate-100">
                      <p className="text-slate-400 font-medium">Units/Year</p>
                      <p className="text-slate-800 font-semibold">12</p>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="p-10 text-center text-slate-400 text-sm">No services found. Create your first service.</div>
          )}
        </div>
      )}
      ...
      {activeTab === 'activate' && (
        <div className="bg-white rounded-b-xl border border-t-0 border-slate-200 p-5">
          <h3 className="text-sm font-semibold text-slate-700 mb-4">Activate Product for Member</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            ...
            <div>
              <label className={LABEL}>Filter District</label>
            ...
            <div>
              <label className={LABEL}>Filter Block</label>
            ...
            {selectedSvc && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3 flex items-center justify-between">
                <div className="text-xs text-emerald-700">
                  {isTree ? `${actForm.treeQty} tree(s) × pricing formula` : `Annual subscription (12 units)`}
                </div>
                <div className="text-base font-bold text-emerald-800">₹{calcPrice.toLocaleString('en-IN')}</div>
              </div>
            )}

            {actMsg && (
              <div className={`text-xs p-3 rounded-lg border ${actMsg.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-red-50 border-red-200 text-red-600'}`}>
                {actMsg.text}
              </div>
            )}

            <button type="submit" disabled={actLoading} className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-sm font-medium py-2.5 rounded-lg transition">
              {actLoading ? 'Activating…' : 'Activate Membership'}
            </button>
          </form>
        </div>
      )}

      {/* Tab: Emergency Request (SuperAdmin on behalf of member via toll-free) */}
      {activeTab === 'emergency' && (
        <div className="bg-white rounded-b-xl border border-t-0 border-slate-200 p-5">
          <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-5 flex items-start gap-2">
            ...
            <p className="text-xs text-red-700">Use this when a member calls on the toll-free number and cannot raise the request themselves. 1 unit will be permanently deducted from their subscription quota.</p>
          </div>
          ...
          <form onSubmit={handleEmergency} className="space-y-4">
            ...
            {emgMsg && (
              <div className={`text-xs p-3 rounded-lg border ${emgMsg.type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-red-50 border-red-200 text-red-600'}`}>
                {emgMsg.text}
              </div>
            )}

            <button type="submit" disabled={emgLoading} className="w-full bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white text-sm font-medium py-2.5 rounded-lg transition">
              {emgLoading ? 'Raising…' : '🚨 Raise Emergency for Member'}
            </button>
          </form>
        </div>
      )}
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl z-10 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleCreateService} className="p-5 space-y-4 overflow-y-auto">
              <div><label className={LABEL}>Service Name *</label>
                <input required className={INPUT} value={svcForm.name} onChange={e => setSvcForm(f => ({...f, name: e.target.value}))} /></div>
              <div><label className={LABEL}>Description *</label>
                <textarea required rows={3} className={INPUT + ' resize-none'} value={svcForm.description} onChange={e => setSvcForm(f => ({...f, description: e.target.value}))} /></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className={LABEL}>Base Fee (₹) *</label>
                  <input required type="number" min="0" className={INPUT} value={svcForm.baseFee} onChange={e => setSvcForm(f => ({...f, baseFee: e.target.value}))} /></div>
                {svcForm.type === 'ON_DEMAND' && (
                  <div><label className={LABEL}>Subsequent Unit (₹)</label>
                    <input type="number" min="0" className={INPUT} value={svcForm.subsequentFee} onChange={e => setSvcForm(f => ({...f, subsequentFee: e.target.value}))} /></div>
                )}
                {svcForm.type === 'SUBSCRIPTION' && (
                  <div><label className={LABEL}>Duration (months)</label>
                    <input type="number" min="1" className={INPUT} value={svcForm.totalMonths} onChange={e => setSvcForm(f => ({...f, totalMonths: e.target.value}))} /></div>
                )}
              </div>
